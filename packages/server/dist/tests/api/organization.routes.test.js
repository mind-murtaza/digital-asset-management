...
describe('Organization API - Comprehensive E2E Tests', () => {
    ...
    beforeAll(async () => {
        ...
        await (0, supertest_1.default)(baseUrl).post('/api/v1/auth/register').send(testUser).expect(201);
        ...
        await (0, supertest_1.default)(baseUrl).post('/api/v1/auth/register').send(secondUser).expect(201);
        const secondLoginRes = await (0, supertest_1.default)(baseUrl)
            .post('/api/v1/auth/login')
            .send({ email: secondUser.email, password: secondUser.password })
            .expect(200);
        secondUserToken = secondLoginRes.body.data.token;
        const secondMeRes = await (0, supertest_1.default)(baseUrl)
            .get('/api/v1/users/me')
    ...
    describe('GET /api/v1/organizations - List Organizations', () => {
        ...
        describe('🔒 Authorization Tests', () => {
            it('rejects request without auth token', async () => {
                await (0, supertest_1.default)(baseUrl).get('/api/v1/organizations').expect(401);
            });
        });
    });
    describe('GET /api/v1/organizations/:id - Get Organization by ID', () => {
        ...
        describe('🔒 Authorization Tests', () => {
            it('rejects request without auth token', async () => {
                await (0, supertest_1.default)(baseUrl).get(`/api/v1/organizations/${orgId}`).expect(401);
            });
        });
    });
        ...
        describe('✅ Valid Update Cases', () => {
            ...
            it('handles partial settings updates', async () => {
                const res = await (0, supertest_1.default)(baseUrl)
                    ...
                    .expect(200);
                expect(res.body.success).toBe(true);
                expect(res.body.data.organization.settings.featureFlags.enablePublicSharing).toBe(false);
            });
    ...
    describe('DELETE /api/v1/organizations/:id - Archive Organization', () => {
        ...
        describe('🔒 Authorization Tests', () => {
            it('rejects request without auth token', async () => {
                await (0, supertest_1.default)(baseUrl).delete(`/api/v1/organizations/${orgId}`).expect(401);
            });
        });
    });
    ...
    describe('🌐 Edge Cases & Error Handling', () => {
        ...
        */
        ...
        it('maintains data consistency across operations', async () => {
            // Create org
            const createRes = await (0, supertest_1.default)(baseUrl)
                .post('/api/v1/organizations')
                .set('Authorization', `Bearer ${authToken}`)
                .send({
            ...
            const getRes = await (0, supertest_1.default)(baseUrl)
            ...
            const archivedRes = await (0, supertest_1.default)(baseUrl)
                .get(`/api/v1/organizations/${newOrgId}`)
                .set('Authorization', `Bearer ${authToken}`)
                .expect(200);
