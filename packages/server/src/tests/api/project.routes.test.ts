...
describe('Project API - Comprehensive E2E Tests', () => {
    ...
    beforeAll(async () => {
        await request(baseUrl).post('/api/v1/auth/register').send(user).expect(201);
    ...
    describe('POST /api/v1/projects - Create Project', () => {
        ...
        it('rejects unauthenticated requests', async () => {
            await request(baseUrl).post('/api/v1/projects').send({}).expect(401);
        });
    });
    ...
    describe('GET /api/v1/projects - List Projects', () => {
        ...
        it('filters by organizationId', async () => {
            const res = await request(baseUrl)
                ...
                .expect(200);
            expect(res.body.success).toBe(true);
            res.body.data.projects.forEach((p: any) => expect(p.organizationId).toBe(orgId));
        });
        ...
        it('rejects unauthenticated requests', async () => {
            await request(baseUrl).get('/api/v1/projects').expect(401);
        });
    });
    ...
    describe('DELETE /api/v1/projects/:id - Soft Delete', () => {
        ...
        it('rejects unauthenticated requests', async () => {
            await request(baseUrl).delete(`/api/v1/projects/${childProjectId}`).expect(401);
        });
    });
});
