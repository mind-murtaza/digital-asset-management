...
describe('Auth API - Comprehensive E2E Tests', () => {
    ...
    describe('POST /api/v1/auth/register - Register', () => {
        ...
        it('rejects duplicate registration', async () => {
            await request(baseUrl).post('/api/v1/auth/register').send(testUser).expect(400);
        });
    ...
    describe('POST /api/v1/auth/login - Login', () => {
        ...
        it('rejects missing fields', async () => {
            await request(baseUrl).post('/api/v1/auth/login').send({}).expect(400);
        });
    });
    ...
    describe('POST /api/v1/auth/refresh - Token Refresh', () => {
        ...
        it('rejects missing bearer token', async () => {
            await request(baseUrl).post('/api/v1/auth/refresh').expect(401);
        });
