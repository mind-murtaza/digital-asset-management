...
describe('User API - Comprehensive E2E Tests', () => {
    ...
    beforeAll(async () => {
        const registerRes = await request(baseUrl).post('/api/v1/auth/register').send(testUser);
    ...
    describe('GET /api/v1/users/me - Current User', () => {
        ...
        it('rejects request without auth token', async () => {
            await request(baseUrl).get('/api/v1/users/me').expect(401);
        });
    });
    ...
    describe('DELETE /api/v1/users/me - Soft Delete', () => {
        ...
        it('rejects request without auth token', async () => {
            await request(baseUrl).delete('/api/v1/users/me').expect(401);
        });
    });
});
