...
* `ownerId` *(User ref, req)*
* `settings.storageQuotaBytes` *(default: 500GB)*
* `settings.featureFlags.enablePublicSharing` *(default: true)*
* `settings.featureFlags.enableApiAccess` *(default: false)*
* `createdAt, updatedAt`
...
* `organizationId` *(Organization ref, req)*
* `name` *(String, req)*
* `description` *(String)*
* `permissions[]` *(e.g., `project:create`, `asset:edit:own`)*
...
* `roleId` *(Role ref, req)*
* `name` *(String, req)*
* `email` *(String, req, unique per org)*
...
* `organizationId` *(Organization ref, req)*
* `name` *(String, req)*
* `path` *(String, hierarchical, req, e.g. `/brand/2025/campaign-a`)*
* `ancestors[]` *({\_id, name})*
* `createdBy` *(User ref, req)*
* `deletedAt` *(Nullable Date)*
* `createdAt, updatedAt`

---

### AssetVersion *(embedded within Asset)*

* `version` *(Number, req)*
* `storageKey` *(String, req)*
* `fileSizeBytes` *(Number, req)*
* `createdBy` *(User ref)*
* `createdAt` *(Date)*

---

### Renditions *(embedded within Asset)*
...
* Status: `status` *(AssetStatus, default: uploading)*, `processingError?`
* Storage: `storageProvider*`, `storageKey*`
* Versions: `latestVersion` *(default: 1)*, `versions[]` *(AssetVersion)*
* Descriptors: `tags[]`, `metadata { width, height, duration, codec, bitrate, pageCount }`, `customMetadata<Map<String,String>>`, `renditions`
* Access: `access` *(AccessLevel, default: private)*
...
* `bullJobId` *(String, req)*
* `assetId` *(Asset ref, req)*
* `organizationId` *(Organization ref, req)*
* `jobName` *(String, req)*
* `status` *(JobStatus, req)*
* `workerId?`, `attempts` *(Number, default: 0)*
* `logs[]` *(JobLog)*, `error?` *(JobError)*
* `queuedAt`, `startedAt`, `completedAt` *(Dates)*, `durationMs` *(Number)*
* `createdAt, updatedAt`
...
* `expiresAt` *(Date, req)*
* `maxDownloads?`, `downloadCount` *(default: 0)*
...
```js
[{ organizationId: 1, name: 1 }, { unique: true }]
...
```js
[{ organizationId: 1, email: 1 }, { unique: true }]
...
[{ organizationId: 1, "ancestors._id": 1 }]
[{ organizationId: 1, deletedAt: 1, updatedAt: -1 }]
```
...
```js
[{ bullJobId: 1 }, { unique: true }]
[{ organizationId: 1, status: 1, queuedAt: -1 }]
...
```js
[{ token: 1 }, { unique: true }]
[{ organizationId: 1, assetId: 1, expiresAt: 1 }]
[{ expiresAt: 1 },
...
* Unique constraints:
  ...
  * `(organizationId, path)` on `Project` *(partial: exclude soft-deleted)*
  * `(organizationId, checksum)` on `Asset` *(partial)*
...
```json
{
  ...
  "latestVersion": 1,
  "versions": [
    { "version": 1, "storageKey": "…/original/v1/hero.jpg", "fileSizeBytes": 2817345, "createdBy": "64b...u9", "createdAt": "2025-08-25T10:12:00Z" }
  ],
  "tags": ["homepage","brand"],
  ...
  "renditions": {
    ...
    "thumbnail_large": { "storageKey": "…/renditions/thumb_l.jpg", "width": 640, "height": 360 },
